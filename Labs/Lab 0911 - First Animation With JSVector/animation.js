...
function init(){
	...
	for(let a = 0; a < 5; a++){
		...
		loc[a] = new JSVector(Math.random() * (window.innerWidth - 2*radius) + radius, Math.random() * (window.innerHeight - 2*radius) + radius);
...
function animate(){
	...
	for(let a = 0; a < 5; a++){
		ctx.beginPath();
		ctx.arc(loc[a].x, loc[a].y, radius, Math.PI*2, 0, false);
		ctx.fill();
		ctx.stroke();
		loc[a].add(vel[a]);
		
		if(loc[a].x < radius || loc[a].x > window.innerWidth - radius) vel[a].x = -vel[a].x;
		if(loc[a].y < radius || loc[a].y > window.innerHeight - radius) vel[a].y = -vel[a].y;
	}
}